...
public class GVector2f implements Serializable{
	...
	public GVector2f getRandomInt(){
		return new GVector2f((float)(x * Math.random()), (float)(y * Math.random()));
	}
	...
	public void forEach(Consumer<GVector2f> action){
		GVector2f vec = new GVector2f();
		for(int i=0 ; i<x ; i++)
			for(int j=0 ; j<y ; j++)
				action.accept(vec.set(i, j));
	}
	...
	public GVector2f max(GVector2f v){
		return new GVector2f(Math.max(v.getX(), getX()), Math.max(v.getY(), getY()));
	}
	...
	public GVector2f min(GVector2f v){
		return new GVector2f(Math.min(v.getX(), getX()), Math.min(v.getY(), getY()));
	}
	...
	public GVector2f set(float x, float y){this.x = x;this.y = y; return this;}
	
	public GVector2f set(GVector2f a){return set(a.x, a.y);}
